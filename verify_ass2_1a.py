...
def unique_rows(rows: List[List[str]]) -> int:
    ...
    for i in range(0, len(rows)):
        if (i <= 0 or list_differs(rows[i], rows[i - 1])) and (
                i >= (len(rows) - 1) or list_differs(rows[i], rows[i + 1])):
            unique += 1
    return unique


# FP, TN, errors = validate_FP(FP_rows, lf1_df, lf2_df, FP, TN)
def validate_FP(rows, lf1_df: pd.DataFrame, lf2_df: pd.DataFrame, FP: int, TN: int, columns: List[str]) -> Tuple[
    int, int, int]:
...
def calculate_statistics(pred_rows: List[List[str]], true_rows: List[List[str]], lf1_df: pd.DataFrame,
                         lf2_df: pd.DataFrame) -> Tuple[int, int, int, int, List[List[str]]]:
    TP, FP, FN, TN = 0, 0, 0, 0
    errors = 0
    FP_rows = []
    i, j = 0, 0
...
try:
    ...
    assert len(student_df.columns) == 2, "Unexpected columns {}".format(
        [col for col in student_df.columns if clean_string(col) not in ['idAbt', 'idBuy']])
    ...
    assert unique_rows(student_rows) == len(student_rows), "DataFrame {} contains duplicate entries".format(
        python_output_name)
