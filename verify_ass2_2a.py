...
try:
    ...
    assert len(student_df.columns) == 4, "Unexpected columns {}.".format(
        [col for col in student_df.columns if clean_string(col) not in ['feature', 'median', 'mean', 'variance']])
    ...
    #    two.
    assert student_rows >= 20, "Solution missing rows (values should be present for all twenty features, only {} rows found).".format(
        student_rows)
    ...
    for feature, data in expected_feature_rows.items():
        print("Found values for '{}'{} - median: {}, mean: {}, variance: {}.".format(data['short'], " " * (
                    maxlen - len(data['short'])), data['values']['median'], data['values']['mean'],
                                                                                     data['values']['variance']))
