...
def calculate_statistics(student_google_df: pd.DataFrame, student_amazon_df: pd.DataFrame, solution_df: pd.DataFrame,
                         sol_google_df: pd.DataFrame, sol_amazon_df: pd.DataFrame) -> Tuple[
    int, int, int, int, int, Dict[str, Union[int, str]]]:
    ...
    for index, row in solution_df.iterrows():
        ...
        blocks_amazon = list(student_amazon_df[student_amazon_df[student_amazon_cols['product_id']] == amazon_row][
                                 student_amazon_cols['block_key']].values)
        blocks_google = list(student_google_df[student_google_df[student_google_cols['product_id']] == google_row][
                                 student_google_cols['block_key']].values)
...
try:
    ...
    assert clean_string('block_key') in [clean_string(col) for col in
                                         student_google_df.columns], "column block_key missing"
    assert clean_string('product_id') in [clean_string(col) for col in
                                          student_google_df.columns], "column product_id missing"
    assert len(student_google_df.columns) == 2, "Unexpected columns {}".format(
        [col for col in student_google_df.columns if clean_string(col) not in ['block_key', 'product_id']])
    ...
    # Check all required columns are present.
    assert clean_string('block_key') in [clean_string(col) for col in
                                         student_amazon_df.columns], "column block_key missing"
    assert clean_string('product_id') in [clean_string(col) for col in
                                          student_amazon_df.columns], "column product_id missing"
    assert len(student_amazon_df.columns) == 2, "Unexpected columns {}".format(
        [col for col in student_amazon_df.columns if clean_string(col) not in ['block_key', 'product_id']])

    timelog("Converting block keys to strings")
    student_google_df[get_cleaned_dicts(student_google_df)['block_key']] = student_google_df[
        get_cleaned_dicts(student_google_df)['block_key']].apply(str)
    student_amazon_df[get_cleaned_dicts(student_amazon_df)['block_key']] = student_amazon_df[
        get_cleaned_dicts(student_amazon_df)['block_key']].apply(str)
    ...
    if not st_amazon_rows >= so_amazon_rows:
        print(
            "Warning: Not all ids have been assigned a block in Buy dataset (amazon_blocks.csv) - {} ids in original, {} ids assigned to blocks in your solution. Ignore this warning if this is intentional.".format(
                so_amazon_rows, st_amazon_rows))
    if not st_google_rows >= so_google_rows:
        print(
            "Warning: Not all ids have been assigned a block in Abt dataset (google_blocks.csv) - {} ids in original, {} ids assigned to blocks in your solution. Ignore this warning if this is intentional.".format(
                so_google_rows, st_google_rows))

    TP, FP, FN, TN, n, largest_block_pair = calculate_statistics(student_google_df, student_amazon_df, solution_df,
                                                                 sol_google_df, sol_amazon_df)
